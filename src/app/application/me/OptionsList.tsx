...
interface OptionsListProps {
  optionName: "values" | "lifestyle" | "personality" | "datingstyle" | "appearance" | "etc"
  step: number
  data: any
  setData: any
}
  ...
  return (
    <OptionsListRoot id="content">
      ...
      <div className="title-box">
        <Typography variant="subtitle2">{step}<span>/6</span></Typography>
      ...
      </div>
      {
        options.map((option, index) => {
          switch (option.me) {
            ...
            case "radio":
              return <RadioOptions key={index} index={index} option={option} data={data} setData={setData} />
            case "chip":
              return <ChipOptions key={index} index={index} option={option} data={data} setData={setData} />
            case "slider":
